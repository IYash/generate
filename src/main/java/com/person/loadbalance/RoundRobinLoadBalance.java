...
public class RoundRobinLoadBalance implements LoadBalance<ServiceProvider>{
    ...
    public ServiceProvider doSelect(List<ServiceProvider> providers) {
        ...
        if(maxWeight >0 && minWeight < maxWeight){//权重不一样
            ...
            List<ServiceProvider> weightProviders = new ArrayList<>();
            for(ServiceProvider p : providers){//筛选权重大于当前权重基数的provider
                if(p.getServiceWeight() > currentWeight){
                    weightProviders.add(p);
                }
            }
            int weightLength = weightProviders.size();
            if(weightLength == 1){
                return weightProviders.get(0);
            }else if(weightLength > 1){
                providers = weightProviders;
